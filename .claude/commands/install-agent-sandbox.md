...
│   └── mcp/            # Python — needs uv (MCP server for Claude.ai)
├── .claude/
...
9. Verify Python apps — uv will auto-install deps on first run:
    ...
    cd apps/direct && uv run direct --help 2>&1 | head -1
    cd apps/mcp && uv sync 2>&1 | tail -1
    ```
...
17. **Direct client** — verify the CLI parses correctly:
    ```
    cd apps/direct && uv run direct --help
    ```
    PASS if help text shows start/get/list/stop commands. FAIL if error.

18. **MCP server** — start the MCP server, confirm it serves, then stop it:
    ```
    cd apps/mcp && uv run agent-mcp &
    MCP_PID=$!
    sleep 2
    curl -s -o /dev/null -w "%{http_code}" http://localhost:7601/mcp
    kill $MCP_PID 2>/dev/null
    ```
    PASS if curl returns an HTTP status (e.g. 400/406 for a bare GET — the port is listening). FAIL if connection refused. Note: this only verifies the server boots locally; remote exposure is a separate Cloudflare tunnel + Access step, not part of this installer.

19. **Justfile** — verify all recipes are visible:
    ```
    just --list
    ```
    PASS if output includes listen, mcp, send, job, jobs, stop. FAIL if any are missing.

20. **Claude Code** — verify it can start (non-interactive):
    ```
    claude --version
    ```
    PASS if version string returned. FAIL if command not found.

21. Remind the user about manual macOS permissions if any steer checks failed:
...
22. Now follow the `Report` section to report the completed work
...
| mcp server | [PASS/FAIL] | [http status or error] |
...
If all 11 pass: "Sandbox is fully operational. Start the server with `just listen`, and the MCP server with `just mcp`."

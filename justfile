...
# Start the MCP server (wraps the listen API for Claude.ai)
mcp:
    cd apps/mcp && uv run agent-mcp

# Send a job to the listen server
send prompt url=default_url:
    cd apps/direct && uv run direct start {{url}} "{{prompt}}"
...
job id url=default_url:
    cd apps/direct && uv run direct get {{url}} {{id}}

# List all jobs (pass --archived to see archived)
jobs *flags:
    cd apps/direct && uv run direct list {{default_url}} {{flags}}

# Show full details of the latest N jobs (default: 1)
latest n="1" url=default_url:
    cd apps/direct && uv run direct latest {{url}} {{n}}

# Stop a running job
stop id url=default_url:
    cd apps/direct && uv run direct stop {{url}} {{id}}

# Archive all jobs
clear url=default_url:
    cd apps/direct && uv run direct clear {{url}}
